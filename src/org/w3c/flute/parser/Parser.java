...
public class Parser implements org.w3c.css.sac.Parser, ParserConstants {
	...
	final public void styleRule() throws ParseException {
		...
		try {
			...
			start = true;
			((CSSDocumentHandler) documentHandler).startSelector(l,
					getLocator());
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case IDENT:
				declaration();
	 ...
	 *                exception during the parse
	 */
	final public Selector simple_selector(Selector selector, char comb)
		...
		if (cond != null) {
			simple_current = selectorFactory.createConditionalSelector(
					simple_current, cond);
		}
		if (selector != null) {
			switch (comb) {
			case ' ':
				...
				break;
			case '+':
				selector = selectorFactory.createDirectAdjacentSelector(
						(short) 1, selector, simple_current);
				break;
			case '>':
				...
				break;
			case '~':
				selector = ((SelectorFactoryImpl) selectorFactory)
						.createAdjacentSelector((short) 1, selector,
								simple_current);
				break;
			default: {
				if (true)
	...
	final public Condition attrib(Condition pred) throws ParseException {
		...
		case 3:
			...
			break;
		case 4:
			c = ((ConditionFactoryImpl) conditionFactory).createCaretCondition(
					name, null, false, attValue);
			break;
		case 5:
			c = ((ConditionFactoryImpl) conditionFactory)
					.createStarIncludesCondition(name, null, false, attValue);
			break;
		case 6:
			c = ((ConditionFactoryImpl) conditionFactory)
					.createEndwithCondition(name, null, false, attValue);
			break;
		default:
			// never reached.
			c = null;
	...
	final public Condition pseudo(Condition pred) throws ParseException {
		...
		case COLON:
			...
			Condition c2 = ((ConditionFactoryImpl) conditionFactory)
					.createPseudoElementCondition(s2);
			if (pred == null) {
				{
					if (true)
						return c2;
				...
				{
					if (true)
						return conditionFactory.createAndCondition(pred, c2);
				}
			}
			// }
		...
		case IDENT:
			...
			if (s.equals("first-letter") || s.equals("first-line")) {
				...
				Condition c = conditionFactory.createPseudoClassCondition(null,
						s);
				if (pred == null) {
					{
						if (true)
		...
		default:
			...
			if (jj_2_6(1)) {
				...
				if (jj_2_4(3)) {
					...
					if (f.equals("lang(")) {
						d = conditionFactory
								.createLangCondition(convertIdent(value.image));
					} else if (f.equals("nth-of-type(")
							|| f.equals("nth-last-of-type(")
							|| f.equals("nth-last-child(")
							|| f.equals("nth-child(")) {
						d = ((ConditionFactoryImpl) conditionFactory)
					...
					default:
						...
						if (jj_2_3(1)) {
							if (jj_2_2(1)) {
								...
								if (getToken(1).image.toLowerCase().startsWith(
										"n")
										|| getToken(1).image.toLowerCase()
												.startsWith("-n")) {
					...
					if (f2.equals("nth-of-type(")
							|| f2.equals("nth-last-of-type(")
							|| f2.equals("nth-last-child(")
							|| f2.equals("nth-child(")) {
						...
						if (value2 != null)
							paranthesesStr += (pl != null ? pl.image : "")
									+ value2.image;
						d2 = ((ConditionFactoryImpl) conditionFactory)
								.createFunctionCondition(
										f2.substring(0, f2.length() - 1),
										paranthesesStr);
					}
					if (pred == null) {
						{
				...
				Condition d3 = null;
				if (f3.equals("not(")) {
					d3 = ((ConditionFactoryImpl) conditionFactory)
							.createNegativeCondition(l, getLocator());
				}
				if (pred == null) {
					{
						if (true)
	...
	final public Condition hash(Condition pred) throws ParseException {
		...
		Condition d = conditionFactory.createIdCondition(n.image.substring(1));
		if (pred == null) {
			{
				if (true)
					return d;
	...
	final public void declaration() throws ParseException {
		...
		try {
			...
			((CSSDocumentHandler) documentHandler).property(name, exp,
					important, getLocator());
	...
	final public LexicalUnitImpl function(char operator, LexicalUnitImpl prev)
		...
		if ("rgba(".equals(f)) {
			...
			if ((i == 7) && loop && (l == null)) {
				{
					if (true)
						return LexicalUnitImpl.createRGBAColor(n.beginLine,
								n.beginColumn, prev, params);
				}
			} else {
				if (errorHandler != null) {
					errorHandler.error(new CSSParseException("RGBA error",
							new LocatorImpl(this, n.beginLine, n.beginColumn)));
				} else {
					{
						if (true)
			...
			while (loop && l != null && i < 5) {
				...
				default: {
					if (true)
						throw new ParseException("hsl implementation error");
				}
				}
				if (loop) {
					l = (LexicalUnitImpl) l.getNextLexicalUnit();
			...
			if ((i == 5) && loop && (l == null)) {
				{
					if (true)
						return LexicalUnitImpl.createHSLColor(n.beginLine,
								n.beginColumn, prev, params);
				}
			} else {
				if (errorHandler != null) {
					errorHandler.error(new CSSParseException("HSL error",
							new LocatorImpl(this, n.beginLine, n.beginColumn)));
				} else {
					{
						if (true)
			...
			while (loop && l != null && i < 7) {
				...
				default: {
					if (true)
						throw new ParseException("hsla implementation error");
				}
				}
				if (loop) {
					l = (LexicalUnitImpl) l.getNextLexicalUnit();
			...
			if ((i == 7) && loop && (l == null)) {
				{
					if (true)
						return LexicalUnitImpl.createHSLAColor(n.beginLine,
								n.beginColumn, prev, params);
				}
			} else {
				if (errorHandler != null) {
	...
	final public LexicalUnitImpl url(LexicalUnitImpl prev)
		...
		if (urlname.charAt(0) == '"' || urlname.charAt(0) == '\u005c'') {
	...
	final public LexicalUnitImpl hexcolor(LexicalUnitImpl prev)
		...
		if (s.length() == 3) {
			...
			first = null;
			// throw new
			// CSSParseException("invalid hexadecimal notation for RGB: " + s,
			// getLocator());
			{
				if (true)
					return LexicalUnitImpl.createIdent(n.beginLine,
							n.beginColumn, prev, n.image);
			}
		}
		...
		{
			if (true)
				return LexicalUnitImpl.createHexColor(n.beginLine,
						n.beginColumn, prev, s);
		}
		throw new Error("Missing return statement in function");
	}
	...
	private boolean jj_3_2() {
		...
		jj_semLA = getToken(1).image.toLowerCase().startsWith("n")
				|| getToken(1).image.toLowerCase().startsWith("-n");
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0x0, 0x62, 0x62, 0x0, 0x60, 0x2, 0x2, 0x2,
				0x13400000, 0x60, 0x2, 0x60, 0x2, 0x0, 0x2, 0x0, 0x2, 0x2, 0x2,
				0x334fbea0, 0x334fbea0, 0x2, 0x20000, 0x2, 0x2, 0x2, 0x2, 0x0,
				0x10000000, 0x2, 0x0, 0x40000, 0x2, 0x0, 0x2, 0x2, 0x2, 0x2,
				0x0, 0x40000, 0x2, 0x0, 0x2, 0x200fbea0, 0x2, 0x2, 0x2,
				0x188000, 0x2, 0x188000, 0x188002, 0x2, 0x2, 0x0, 0x40000, 0x2,
				0x0, 0x2, 0x20000, 0x2, 0x2, 0x12400000, 0x12400000,
				0x12400000, 0x12400000, 0x12400000, 0x12400000, 0x12400000,
				0x12400000, 0x12400000, 0x12400000, 0x13400000, 0x1000000, 0x2,
				0x2, 0x7e00, 0x2, 0x0, 0x2, 0x7e00, 0x2, 0x2, 0x2, 0x8000, 0x0,
				0x0, 0x0, 0x0, 0x2, 0x2, 0x10000000, 0x2, 0x0, 0x2, 0x2, 0x2,
				0x220000, 0x238000, 0x220000, 0x18000, 0x18000, 0x0, 0x0,
				0x18000, 0x2, 0x2, 0x18000, 0x2, 0x13400000, 0x2, 0x2, 0x2,
				0x0, 0x40000, 0x2, 0x0, 0x2, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x0, 0x0, 0x0, 0x80000000, 0x0, 0x0, 0x0, 0x0,
				0x40000100, 0x0, 0x0, 0x0, 0x0, 0x880, 0x0, 0x100, 0x0, 0x0,
				0x0, 0xc0001b80, 0xc0001b80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x100, 0x0, 0x0, 0x100, 0x0, 0x0, 0x100, 0x0, 0x0, 0x0, 0x0,
				0x100, 0x0, 0x0, 0x100, 0x0, 0x80001a80, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x100, 0x0, 0x0, 0x100, 0x0, 0x0, 0x0,
				0x0, 0x40000000, 0x40000000, 0x0, 0x0, 0x40000000, 0x40000000,
				0x40000000, 0x40000000, 0x40000000, 0x40000000, 0x40000100,
				0x100, 0x0, 0x0, 0x0, 0x0, 0x180, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x20000000, 0x40, 0x240, 0x200, 0x0, 0x0, 0x100, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x7ffffb80, 0x0, 0x0, 0x0, 0x3ffff200,
				0x40000980, 0x7ffffb80, 0x0, 0x0, 0x7ffffb80, 0x0, 0xc0000100,
				0x0, 0x0, 0x0, 0x100, 0x0, 0x0, 0x100, 0x0, };
	}

	private static void jj_la1_init_2() {
		jj_la1_2 = new int[] { 0x2, 0x10, 0x10, 0x0, 0x10, 0x0, 0x0, 0x0, 0xd,
				0x10, 0x0, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3803f,
				0x3803f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3803e,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x20, 0x0, 0x0, 0x0, 0x0, 0x18000, 0x0, 0x0, 0x0, 0x10000,
				0x8000, 0x18000, 0x0, 0x0, 0x18000, 0x0, 0xd, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, };
	}
